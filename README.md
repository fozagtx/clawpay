# ClawPay — Solana Pay invoices for ZeroClaw agents
...
Talking to their agent — typically over WhatsApp — a street vendor, delivery worker or freelancer can:

- **Create a stablecoin invoice** (Solana Pay link + QR content, USDC by default)
- **Detect payment on-chain** and get a confirmation in natural Brazilian Portuguese
- **Check the status** of any invoice — paga, pendente, parcial ou expirada
- **Optionally sweep a hard-capped percentage** of what they receive into a pre-approved yield wallet (“reserva de rendimento”)
...
Amounts are denominated in the invoice token (USDC) and formatted Brazilian-style (`1.234,56`). Converting to/displaying BRL is the agent's job; the plugin never invents an exchange rate.
...
| Key | Default | Meaning |
|---|---|---|
| `recipient` | — | Merchant wallet (base58). Invoices are payable to this address. |
...
| `yield_destination` | — | Pre-approved yield wallet. Sweeps refuse without it. Its token account must exist. |
...
- `sweep_yield` — same lookup fields plus `pct`. On success returns `sweep_ready` with `unsigned_tx_base64` (a legacy-format SPL `TransferChecked`, empty signature slot, recent blockhash — sign and submit within ~1–2 minutes) and `message_pt` (“Separei 10% (15,00 USDC)…”).
...
Direct Pix generation, KYC/banking, multi-signature escrow, cross-chain, recurring invoices, and any UI outside the agent chat.
