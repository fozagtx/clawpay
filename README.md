...
- **Concurrent prepared sweeps are serialized on-chain when `nonce_account` is set.** Each sweep transaction is then built on that durable nonce with an `AdvanceNonceAccount` first instruction, so of several prepared-but-unsigned sweeps at most one can ever confirm; the rest die when the nonce advances. A deferred nonce sweep debits the cap of the day it lands (preparations after it confirms count it), and rotating nonce accounts voids the one-outstanding-sweep guarantee. Without a nonce account the daily cap only counts confirmed transactions, so prepare and sign one sweep at a time.
...
| `invoice_secret` | (none) | Signs invoice tickets (HMAC). Sweeps refuse without it. Use a long random string (16 bytes minimum). |
...
```bash
cargo test           # 60 tests: money/date handling, wire format, status matrix,
